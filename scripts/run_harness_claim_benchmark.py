"""Run the Claim Benchmark v1 for Adaptive Skill System.

This script is the release-grade benchmark runner.  It is intentionally modelled
after ``run_harness_real_benchmark.py`` but targets the independent
``claim_benchmark_suite`` (18 cases, L1×6 / L2×6 / L3×6).

Key differences from real-benchmark runner:
- Imports from ``claim_benchmark_suite`` instead of ``benchmark_suite``.
- Different default output paths and labels.
- ``--baseline-id`` defaults to ``claim-benchmark-v1``.
...
Usage examples::
    ...
    # Lock a new baseline
    python scripts/run_harness_claim_benchmark.py \\
        --baseline-out harness_baselines/claim-benchmark-v1.json
...
DEFAULT_OUTPUT_DIR = ".benchmark-artifacts/claim-benchmark"
DEFAULT_BATCH_FILE = "claim-benchmark-batch-result.json"
DEFAULT_METRICS_FILE = "claim-benchmark-metrics.json"
DEFAULT_FILE_STEM = "adaptive-skill-harness-claim-benchmark"
DEFAULT_REPORT_TITLE = "Adaptive Skill Harness Claim Benchmark v1"
DEFAULT_BASELINE_ID = "claim-benchmark-v1"
DEFAULT_BASELINE_LABEL = "claim-benchmark-v1"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "运行 claim-benchmark-v1（18-case seeded suite），"
...
def _build_baseline(
    ...
    *,
    ...
    return BaselineRecord(
        ...
        metrics=metrics,
        notes=(
            "Locked from the claim-benchmark-v1 suite generated by "
            "scripts/run_harness_claim_benchmark.py.  "
            "This baseline is the release-grade evidence source — "
            "do NOT replace with data from ci-smoke or real-benchmark runs."
        ),
        metadata={
            "suite": CLAIM_BENCHMARK_SUITE_ID,
            "generated_by": "scripts/run_harness_claim_benchmark.py",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.baseline_out:
        ...
        _write_json(baseline_path, rec.to_dict())
    ...
    print("Claim Benchmark v1 completed:")
...
def _wilson_lower(k: int, n: int, z: float = 1.96) -> float:
    """Return the lower bound of the Wilson score interval (95% CI by default)."""
    if n == 0:
        return 0.0
